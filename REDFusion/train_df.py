...
def get_args(parser):
    ...
    parser.add_argument("--model", type=str, default="bow", choices=["bow", "img", "bert", "concatbow", "concatbert", "mmbt", "latefusion", "early_wd"])
    ...
    parser.add_argument("--task", type=str, default="mmimdb", choices=["mmimdb", "vsnli", "food101","MVSA_Single","MVSA_multiple","HFM"])
    ...
    parser.add_argument("--before_clf_size", type=int, default=1024)
    parser.add_argument("--swd_weight", type=float, default=0.01)
...
def rank_loss(confidence, idx, history):
    ...
    # ranking loss
    ranking_loss = nn.MarginRankingLoss(margin=0.0)(rank_input1, rank_input2, -rank_target.reshape(-1,1))
    # max(0, -rank_target.reshape(-1,1) * (rank_input1-rank_input2) + margin)

    return ranking_loss
def model_forward(i_epoch, model, args, criterion, batch,txt_history=None,img_history=None,mode='eval'):
    txt, segment, mask, img, tgt, idx = batch  # txt: [16, 60], img: [16, 3, 224, 224]
    ...
    elif args.model == "early_wd":

        txt, img = txt.cuda(), img.cuda()
        mask, segment = mask.cuda(), segment.cuda()
        
        txt_img_logits, txt_logits, img_logits, txt_conf, img_conf, txt_out, img_out = model(txt, mask, segment, img)
    ...
    # main_clf_loss = nn.CrossEntropyLoss()(txt_img_logits, tgt)

    swd_loss = sliced_wasserstein_distance(txt_out, img_out)
    ...
    swd_loss_detach = swd_loss.detach()

    if mode == 'train':
        txt_history.correctness_update(idx, txt_loss, txt_conf.squeeze())  # idx, txt_loss, 以及 txt_conf.squeeze() 都是 [16]
        img_history.correctness_update(idx, img_loss + 0.001 * swd_loss_detach, img_conf.squeeze())
        ...
        # loss = torch.mean(main_clf_loss + crl_loss) + args.swd_weight * swd_loss
        loss = torch.mean(clf_loss + crl_loss) + args.swd_weight * swd_loss
...
def train(args):
    ...
    os.makedirs(args.savedir, exist_ok=True)  # exist_ok只有在目录存在时候才创建
    ...
    # print('1341232')
    criterion = get_criterion(args)  # 在 MVSA-single 的情况下是 classification
    optimizer = get_optimizer(model, args)  # late_fusion, lr = 5e-05
