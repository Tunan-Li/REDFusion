...
def get_vocab(args):
    vocab = Vocab()
    if args.model in ["bert", "mmbt", "concatbert", "latefusion", 'tmc', "early_wd"]:
...
def collate_fn(batch, args):
    ...
    img_tensor = None
    if args.model in ["img", "concatbow", "concatbert", "mmbt", "latefusion", 'tmc', "early_wd"]:
...
def get_data_loaders(args):
    ...
    tokenizer = (
        ...
        BertTokenizer.from_pretrained('bert-base-uncased', do_lower_case=True).tokenize
        if args.model in ["bert", "mmbt", "concatbert", "latefusion", 'tmc', "early_wd"]
        else str.split
    )
    ...
    # ./datasets/MVSA_Single/train.jsonl

    args.labels, args.label_freqs = get_labels_and_frequencies(os.path.join(args.data_path, args.task, "train.jsonl"))

    # label = ['positive', 'neutral', 'negative'] label.freqs = Counter({'positive': 851, 'negative': 422, 'neutral': 282})

    vocab = get_vocab(args)
    args.vocab = vocab
    args.vocab_sz = vocab.vocab_sz  # 30522
    args.n_classes = len(args.labels)  # 3
